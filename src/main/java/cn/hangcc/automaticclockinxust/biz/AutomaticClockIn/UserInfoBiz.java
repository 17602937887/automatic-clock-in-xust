...
public class UserInfoBiz {
    ...
    public String getUserCookie(String url) throws IOException {
        try {
            ...
            // 创建请求 并设置headers
            HttpGet request = AutomaticClockInUtils.setHeaders(new HttpGet(url));
            // 执行http请求
    ...
    public UserInfoModel getUserInfo(String url, Long schoolId) {
        try {
            ...
            CloseableHttpClient client = HttpClientBuilder.create().build();
            // 设置请求地址及Headers
            HttpGet request = AutomaticClockInUtils.setHeaders(new HttpGet(String.format("%s%d", AutomaticClockInConstants.REQUEST_USER_INFO_URL, schoolId)));
            // 设置Cookie
